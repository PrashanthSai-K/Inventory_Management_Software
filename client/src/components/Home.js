          ...
          {Menus.map((Menu, index) => (
			...
			<div className={`${open ? "w-64" : "w-20"} h-screen bg-blue-800 w-1/6 fixed left-0 right-0 navbar duration-300`} >
				<img src="/images/control.png" alt=""
					className={`absolute -right-3 top-9 w-8 border-blue-800 border-2  rounded-full ${!open && "rotate-180"} `}
					onClick={() => setOpen(!open)}
				/>

				<div className="flex gap-x-4  items-center ml-3 mt-10 font">
					<img src="/images/bit1.png" alt="" className={`duration-300 w-10 ${open && "rotate-[360deg]"}`} />
					<h1 className={`ml-2 mb-2.5 text-3xl pt-1 ${!open && "hidden"}`}>Stores</h1>
				</div>
				<div className="mt-10 ml-2 mr-2 h-screen" style={{ fontSize: "21px" }}>
					<ul>
						<li className="flex gap-x-4 mb-4 cursor-pointer hover:bg-gray-700 rounded-full  pl-4 pt-1 pr-2 pb-2">
							<i className={`bi bi-speedometer ${!open && "text-3xl text-center"} duration-300 `}></i>
							<span className={` duration-300 ${!open && "hidden"}`}> Dashboard</span>
						</li>
						<li className="flex gap-x-4 mb-4 cursor-pointer hover:bg-gray-700 rounded-full  pl-4 pt-1 pr-2 pb-2">
							<i className={`bi bi-file-person-fill  ${!open && "text-3xl text-center"} duration-300`}></i>
							<span className={` duration-300 ${!open && "hidden"}`}> Master</span>
						</li>
						<li className="flex gap-x-4 mb-4 cursor-pointer hover:bg-gray-700 rounded-full  pl-4 pt-1 pr-2 pb-2">
							<i class={`bi bi-archive-fill ${!open && "text-3xl text-center"} duration-300`}></i>
							<span className={` duration-300 ${!open && "hidden"}`}> Supplier</span>
						</li>
						<li className="flex gap-x-4 mb-4 cursor-pointer hover:bg-gray-700 rounded-full  pl-4 pt-1 pr-2 pb-2">
							<i className= {`bi bi-building ${!open && "text-3xl text-center"} duration-300`}></i>
